<?php $__env->startSection('title', 'TicketIn — Temukan Event Terbaik'); ?>
...
<section class="pt-[96px] pb-6 max-w-7xl mx-auto px-6">
  <div class="relative w-full rounded-2xl overflow-hidden shadow-md group" style="max-height:400px;aspect-ratio:21/9;">
    ...
    <div id="slider" class="slider-track h-full">
      <?php if($eventsSlider->isNotEmpty()): ?>
      <?php $__currentLoopData = $eventsSlider; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $slide): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <?php
      ...
      ?>
      <div class="slide h-full cursor-pointer relative" onclick="location.href='<?php echo e($slideLink); ?>'">
        <img src="<?php echo e($slideImg); ?>"
          onerror="this.src='https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1400&q=80'"
          alt="<?php echo e($slideTitle); ?>" class="w-full h-full object-cover" />
        <?php if($slideTitle): ?>
        <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent px-8 py-6">
          <p class="text-white font-bold text-xl drop-shadow"><?php echo e($slideTitle); ?></p>
        </div>
        <?php endif; ?>
      </div>
    ...
    <div id="dots" class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10">
      <?php $slideCount = $eventsSlider->isNotEmpty() ? $eventsSlider->count() : 3; ?>
      <?php for($i = 0; $i < $slideCount; $i++): ?>
        <button class="dot w-2.5 h-2.5 rounded-full <?php echo e($i==0?'bg-white active':'bg-white/50'); ?>" data-index="<?php echo e($i); ?>"></button>
        <?php endfor; ?>
    </div>
  </div>
</section>
...
<section class="max-w-7xl mx-auto px-6 py-14">
  <div class="flex items-center justify-between mb-8">
    ...
    <a href="<?php echo e(route('events.index')); ?>" class="text-navy-mid font-semibold hover:text-gold transition-colors text-sm flex items-center gap-1">
      Lihat semua
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
    <?php if($eventsTerdekat->isNotEmpty()): ?>
    <?php $__currentLoopData = $eventsTerdekat; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <div class="event-card bg-white rounded-xl shadow-md overflow-hidden cursor-pointer group flex flex-col"
      style="height:380px;"
      onclick="location.href='<?php echo e(route('events.show', $row)); ?>'">
      <div class="overflow-hidden flex-shrink-0" style="height:180px;">
        <img src="<?php echo e($row->cover_url); ?>"
      ...
      <div class="p-4 flex flex-col flex-1 min-h-0">
        <div class="mb-2">
          <span class="bg-navy-mid/10 text-navy-mid text-xs px-2 py-0.5 rounded-full font-medium"><?php echo e($row->kategori); ?></span>
        </div>
        ...
        <p class="text-xs text-gray-400 flex items-center gap-1 mb-0.5">
          ...
          <?php echo e($row->tanggal_waktu->format('d M Y, H:i')); ?>
        ...
        <p class="text-xs text-gray-400 flex items-center gap-1">
          ...
          <?php echo e($row->lokasi_kota); ?>

        </p>
        <div class="flex-1"></div>
        <div class="flex items-center justify-between pt-3 border-t border-gray-100">
          <p class="text-navy-mid font-bold text-sm">
            <?php echo e(($row->harga_termurah ?? 0) == 0 ? 'GRATIS' : 'Rp '.number_format($row->harga_termurah, 0, ',', '.')); ?>
...
<section class="max-w-7xl mx-auto px-6 py-14">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
    <?php if($eventsRekomendasi->isNotEmpty()): ?>
    <?php $__currentLoopData = $eventsRekomendasi; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ticketin-laravel\resources\views/dashboard/index.blade.php ENDPATH**/ ?>