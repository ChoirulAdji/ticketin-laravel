...
<div class="p-6 max-w-7xl mx-auto">
  ...
  <div class="flex items-center justify-between mb-6">
    <div>
      ...
      <p class="text-gray-400 text-sm mt-0.5">Total <?php echo e($events->total()); ?> event</p>
    </div>
    <form method="GET" class="flex gap-2">
      <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari judul event..."
  ...
  <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
    ...
    <?php else: ?>
            ...
            <tr class="hover:bg-gray-50 transition-colors">
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <img src="<?php echo e($event->cover_url); ?>" class="w-12 h-10 rounded-xl object-cover flex-shrink-0">
                  <div class="min-w-0">
                    <p class="font-semibold text-navy-deep text-sm truncate max-w-[180px]"><?php echo e($event->judul); ?></p>
                    <p class="text-gray-400 text-xs"><?php echo e($event->kategori); ?> · <?php echo e($event->lokasi_kota); ?></p>
                  </div>
                </div>
              </td>
              <td class="px-4 py-4">
                <div class="flex items-center gap-2">
                  <img src="<?php echo e($event->pengelola->avatar_url); ?>" class="w-7 h-7 rounded-full object-cover">
                  <span class="text-xs text-gray-600 truncate max-w-[120px]"><?php echo e($event->pengelola->nama_panggilan); ?></span>
                </div>
              </td>
              <td class="px-4 py-4">
                <p class="text-xs text-gray-600"><?php echo e($event->tanggal_waktu->format('d M Y')); ?></p>
                <p class="text-xs text-gray-400"><?php echo e($event->tanggal_waktu->format('H:i')); ?> WIB</p>
              </td>
              <td class="px-4 py-4">
                <p class="text-xs font-bold text-navy-deep"><?php echo e($event->orders_count); ?></p>
                <p class="text-xs text-gray-400">pesanan</p>
              </td>
              <td class="px-4 py-4">
                <span class="text-xs font-semibold px-2 py-1 rounded-full
                  <?php echo e($event->status==='published'?'badge-published':($event->status==='draft'?'badge-draft':'badge-cancelled')); ?>">
              ...
              <td class="px-4 py-4">
                <div class="flex items-center gap-2">
                  ...
                  <form method="POST" action="<?php echo e(route('admin.events.hapus', $event)); ?>"
                        onsubmit="return confirm('Hapus event <?php echo e(addslashes($event->judul)); ?>?')">
      ...
      <div class="px-6 py-4 border-t border-gray-100"><?php echo e($events->links()); ?></div>
    <?php endif; ?>
  </div>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Application\Software\XAMPP\htdocs\ticketin-laravel\resources\views/admin/events.blade.php ENDPATH**/ ?>