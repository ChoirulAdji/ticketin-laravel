<?php $__env->startSection('title', 'Profil Saya — TicketIn'); ?>
...
<div class="pt-24 max-w-5xl mx-auto px-6 py-10">
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
    <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6">
      <div class="relative flex-shrink-0">
        <img src="<?php echo e($user->avatar_url); ?>" class="w-24 h-24 rounded-2xl object-cover shadow-sm bg-gray-100">
        <a href="<?php echo e(route('profile.edit')); ?>" class="absolute -bottom-2 -right-2 w-8 h-8 bg-gold rounded-full flex items-center justify-center shadow hover:bg-gold-light transition-all">
      ...
      <div class="flex-1 text-center sm:text-left">
        <h1 class="text-2xl font-extrabold text-navy-deep"><?php echo e($user->nama_lengkap); ?></h1>
        <p class="text-gray-500 text-sm mt-1"><?php echo e($user->email); ?></p>
        <?php if($user->no_hp): ?><p class="text-gray-400 text-sm">+62<?php echo e($user->no_hp); ?></p><?php endif; ?>
        <div class="flex items-center justify-center sm:justify-start gap-2 mt-3">
          <span class="text-xs font-bold px-3 py-1 rounded-full <?php echo e($user->role==='pengelola' ? 'bg-gold/20 text-yellow-700 border border-gold/40' : ($user->role==='admin' ? 'bg-red-100 text-red-700 border border-red-300' : 'bg-navy-mid/10 text-navy-mid border border-navy-mid/20')); ?>">
            <?php echo e($user->role==='pengelola' ? ' Pengelola Event' : ($user->role==='admin' ? ' Admin' : ' Pembeli')); ?>

          </span>
          <span class="text-gray-400 text-xs">Bergabung <?php echo e($user->created_at->format('M Y')); ?></span>
        </div>
      </div>
      <div class="flex gap-3 flex-shrink-0">
        <a href="<?php echo e(route('profile.edit')); ?>" class="bg-gold text-navy-deep font-bold px-4 py-2 rounded-xl hover:bg-gold-light transition-all text-sm shadow-sm">Edit Profil</a>
        <?php if($user->isPengelola()): ?>
        <a href="<?php echo e(route('pengelola.dashboard')); ?>" class="bg-navy-mid text-white font-bold px-4 py-2 rounded-xl hover:bg-navy-deep transition-all text-sm">Dashboard EO</a>
        <?php endif; ?>
      </div>
    </div>
    <div class="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-gray-100">
      <div class="text-center">
        <p class="text-2xl font-extrabold text-navy-deep"><?php echo e($orders->count()); ?></p>
        <p class="text-gray-400 text-xs mt-0.5">Total Pesanan</p>
      </div>
      <div class="text-center">
        <p class="text-2xl font-extrabold text-gold"><?php echo e($orders->where('status','paid')->count()); ?></p>
        <p class="text-gray-400 text-xs mt-0.5">Tiket Aktif</p>
      </div>
      <div class="text-center">
        <p class="text-2xl font-extrabold text-navy-deep"><?php echo e($orders->sum('total_qty')); ?></p>
        <p class="text-gray-400 text-xs mt-0.5">Total Tiket</p>
      </div>
    </div>
    <div class="grid grid-cols-1 mt-4 pt-4 border-t border-gray-100">
      <div class="text-center">
        <p class="text-2xl font-extrabold text-red-400"><?php echo e($wishlists->count()); ?></p>
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="tab-panel active p-6" id="tab-tiket">
      <?php $paidOrders = $orders->where('status','paid'); ?>
      <?php if($paidOrders->isEmpty()): ?>
      <div class="py-16 text-center">
        ...
        <a href="<?php echo e(route('events.index')); ?>" class="inline-block bg-gold text-navy-deep font-bold px-6 py-3 rounded-xl text-sm hover:bg-gold-light transition-all">Cari Event Sekarang</a>
      </div>
      <?php else: ?>
      <div class="space-y-6">
        ...
        <div class="eticket-wrap">
          <div class="ticket-body shadow-xl">
            <div class="p-5 pb-0">
              <div class="flex justify-between items-start mb-3">
                <div class="flex-1 min-w-0 pr-4">
                  ...
                  <h3 class="text-lg font-extrabold text-white leading-tight line-clamp-2"><?php echo e($order->event->judul); ?></h3>
                </div>
              ...
              <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                <div class="bg-white/5 rounded-xl p-3">
                  ...
                  <p class="text-white font-bold text-xs"><?php echo e($order->event->tanggal_waktu->format('d M Y')); ?></p>
                </div>
                <div class="bg-white/5 rounded-xl p-3">
                  <p class="text-white/40 text-xs mb-1"> Waktu</p>
                  <p class="text-white font-bold text-xs"><?php echo e($order->event->tanggal_waktu->format('H:i')); ?> WIB</p>
                </div>
                <div class="bg-white/5 rounded-xl p-3">
                  <p class="text-white/40 text-xs mb-1"> Lokasi</p>
                  <p class="text-white font-bold text-xs truncate"><?php echo e($order->event->lokasi_kota); ?></p>
                </div>
                <div class="bg-white/5 rounded-xl p-3">
                  <p class="text-white/40 text-xs mb-1"> Jumlah</p>
                  <p class="text-white font-bold text-xs"><?php echo e($order->total_qty); ?> Tiket</p>
                </div>
              </div>
              <div class="flex flex-wrap gap-2 mb-4">
                <?php $__currentLoopData = $order->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <span class="bg-gold/20 text-gold border border-gold/30 text-xs font-semibold px-3 py-1 rounded-full"><?php echo e($item->qty); ?>x <?php echo e($item->ticketCategory->nama_kategori); ?></span>
            ...
            <div class="p-5 pt-0 flex items-center justify-between gap-4">
              <div class="flex-1 min-w-0">
                ...
                <p class="text-white font-bold text-sm truncate"><?php echo e($user->nama_lengkap); ?></p>
                <p class="text-white/40 text-xs mt-2 mb-0.5">Kode Tiket</p>
                <p class="font-mono font-extrabold text-gold tracking-widest text-sm"><?php echo e($order->order_code); ?></p>
                <p class="text-white/30 text-xs mt-3">Venue: <?php echo e($order->event->venue); ?></p>
              </div>
              <div class="flex flex-col items-center gap-2 flex-shrink-0">
                <div class="qr-canvas" id="qr-<?php echo e($order->order_code); ?>"></div>
          ...
          <div class="flex gap-2 mt-3">
            <button onclick="lihatTiket(<?php echo e($order->id); ?>)" class="flex-1 bg-navy-mid text-white font-semibold py-2.5 rounded-xl hover:bg-navy-deep transition-all text-sm flex items-center justify-center gap-2">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
              Lihat Detail
            </button>
            <button onclick="printTiket(<?php echo e($order->id); ?>)" class="flex-1 bg-white border border-gray-200 text-gray-700 font-semibold py-2.5 rounded-xl hover:bg-gray-50 transition-all text-sm flex items-center justify-center gap-2">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
              Print
            </button>
            <button onclick="bagikanTiket('<?php echo e($order->order_code); ?>', '<?php echo e(addslashes($order->event->judul)); ?>')" class="bg-gold text-navy-deep font-semibold py-2.5 px-4 rounded-xl hover:bg-gold-light transition-all text-sm flex items-center justify-center gap-2">
          ...
          <div id="order-data-<?php echo e($order->id); ?>" class="hidden"
               data-judul="<?php echo e($order->event->judul); ?>"
               data-tanggal="<?php echo e($order->event->tanggal_waktu->format('d M Y')); ?>"
               data-waktu="<?php echo e($order->event->tanggal_waktu->format('H:i')); ?>"
               data-lokasi="<?php echo e($order->event->lokasi_kota); ?>"
               data-venue="<?php echo e($order->event->venue); ?>"
               data-qty="<?php echo e($order->total_qty); ?>"
               data-summary="<?php echo e($order->ticket_summary); ?>"
               data-kode="<?php echo e($order->order_code); ?>"
               data-nama="<?php echo e($user->nama_lengkap); ?>"
               data-total="Rp <?php echo e(number_format($order->total_harga,0,',','.')); ?>"
               data-cover="<?php echo e($order->event->cover_url); ?>">
          </div>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </div>
    ...
    <div class="tab-panel p-6" id="tab-riwayat">
      ...
      <div class="py-16 text-center">
        ...
        <a href="<?php echo e(route('events.index')); ?>" class="inline-block bg-gold text-navy-deep font-bold px-6 py-3 rounded-xl text-sm">Jelajahi Event</a>
      </div>
      <?php else: ?>
      <div class="space-y-4">
        ...
        <div class="bg-gray-50 border border-gray-100 rounded-2xl p-5 hover:border-navy-mid/20 transition-colors">
          <div class="flex flex-col sm:flex-row items-start gap-4">
            <img src="<?php echo e($order->event->cover_url); ?>" class="w-16 h-16 rounded-xl object-cover flex-shrink-0">
            <div class="flex-1 min-w-0">
              <p class="font-bold text-navy-deep text-sm truncate"><?php echo e($order->event->judul); ?></p>
              ...
              <p class="text-gray-500 text-xs mt-1"><?php echo e($order->ticket_summary); ?></p>
              <div class="flex items-center gap-3 mt-2">
                <span class="font-mono text-xs text-navy-mid font-bold bg-navy-mid/10 px-2 py-0.5 rounded-lg"><?php echo e($order->order_code); ?></span>
                <span class="text-gray-300 text-xs"><?php echo e($order->created_at->format('d M Y, H:i')); ?></span>
              </div>
            </div>
            <div class="flex sm:flex-col items-center sm:items-end gap-3 flex-shrink-0 w-full sm:w-auto">
              <span class="text-xs font-semibold px-3 py-1 rounded-full <?php echo e($order->status==='paid' ? 'badge-paid' : ($order->status==='pending' ? 'badge-pending' : 'badge-cancelled')); ?>">
                <?php echo e($order->status==='paid' ? ' Lunas' : ($order->status==='pending' ? ' Menunggu Bayar' : ' Dibatalkan')); ?>
            ...
            <div class="flex flex-col sm:flex-row gap-3">
              <div class="flex-1 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-yellow-600 text-xs">Via <?php echo e($order->metode_bayar); ?></p>
                </div>
              </div>
              <div class="flex gap-2">
                <form method="POST" action="<?php echo e(route('profile.cek-bayar', $order)); ?>">
                  <?php echo csrf_field(); ?>
                ...
                <button onclick="konfirmasiBatal('<?php echo e($order->order_code); ?>', <?php echo e($order->id); ?>)" class="bg-red-50 hover:bg-red-100 text-red-600 font-bold px-4 py-2.5 rounded-xl transition-all text-sm flex items-center gap-2 border border-red-200 whitespace-nowrap">
    ...
    <div class="tab-panel p-6" id="tab-favorit">
      <?php if($wishlists->isEmpty()): ?>
        <div class="py-16 text-center">
          ...
          <a href="<?php echo e(route('events.index')); ?>" class="inline-block bg-gold text-navy-deep font-bold px-6 py-3 rounded-xl text-sm hover:bg-gold-light transition-all">Jelajahi Event</a>
        </div>
      <?php else: ?>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <?php $__currentLoopData = $wishlists; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fav): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <a href="<?php echo e(route('events.show', $fav)); ?>"
            ...
            <div class="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 bg-gray-200">
              <img src="<?php echo e($fav->cover_url); ?>"
                   onerror="this.src='https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=400&q=80'"
                   alt="<?php echo e($fav->judul); ?>"
                   class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
            </div>
            <div class="flex-1 min-w-0">
              <span class="text-xs font-semibold text-navy-mid bg-navy-mid/10 px-2 py-0.5 rounded-full"><?php echo e($fav->kategori); ?></span>
              <p class="font-bold text-navy-deep text-sm mt-1.5 leading-snug line-clamp-2"><?php echo e($fav->judul); ?></p>
              <p class="text-xs text-gray-400 mt-1 flex items-center gap-1">
                ...
                <?php echo e($fav->tanggal_waktu->format('d M Y')); ?>

              </p>
              <p class="text-xs text-gray-400 flex items-center gap-1">
                <svg class="w-3 h-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                <?php echo e($fav->lokasi_kota); ?>

              </p>
              <p class="text-xs font-bold text-navy-mid mt-1">
                <?php if($fav->harga_termurah == 0): ?> <span class="text-green-600">GRATIS</span>
                <?php else: ?> Rp <?php echo e(number_format($fav->harga_termurah, 0, ',', '.')); ?> <?php endif; ?>
              </p>
            </div>
            <div class="flex-shrink-0 self-start">
              <button
                class="wish-toggle w-7 h-7 rounded-full bg-red-50 flex items-center justify-center hover:bg-red-100 transition"
                data-event-id="<?php echo e($fav->id); ?>"
                data-url="<?php echo e(route('wishlist.toggle', $fav)); ?>"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ticketin-laravel\resources\views/profile/index.blade.php ENDPATH**/ ?>