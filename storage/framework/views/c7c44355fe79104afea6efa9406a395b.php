<?php $__env->startSection('title', $event ? 'Edit Event' : 'Tambah Event'); ?>
...
<div class="max-w-3xl mx-auto px-6 py-10">
  <div class="flex items-center gap-3 mb-8">
    ...
    <h1 class="text-2xl font-extrabold text-navy-deep"><?php echo e($event ? 'Edit Event' : 'Tambah Event Baru'); ?></h1>
  </div>
  ...
  <form method="POST" action="<?php echo e($event ? route('pengelola.event.update',$event) : route('pengelola.event.store')); ?>"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
      ...
      <div>
        ...
        <input type="text" name="judul" value="<?php echo e(old('judul',$event->judul??'')); ?>"
               placeholder="Nama event kamu" class="form-input px-4 py-3 text-sm" required>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <select name="kategori" class="form-input px-4 py-3 text-sm">
            <?php $__currentLoopData = ['Konser','Festival','Seminar','Workshop','Olahraga','Seni','Teknologi','Lainnya']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <option value="<?php echo e($kat); ?>" <?php echo e(old('kategori',$event->kategori??'')===$kat?'selected':''); ?>><?php echo e($kat); ?></option>
        ...
        <div>
          ...
          <div class="form-input px-4 py-3 text-sm bg-amber-50 border-amber-200 text-amber-700 font-medium">
             Menunggu Review Admin — event akan tampil ke publik setelah disetujui admin
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="date" name="tanggal"
                 value="<?php echo e(old('tanggal',$event?$event->tanggal_waktu->format('Y-m-d'):'')); ?>"
                 class="form-input px-4 py-3 text-sm" required>
        </div>
        <div>
          <label class="form-label">Jam Mulai</label>
          <input type="time" name="waktu"
                 value="<?php echo e(old('waktu',$event?$event->tanggal_waktu->format('H:i'):'')); ?>"
                 class="form-input px-4 py-3 text-sm" required>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="lokasi_kota" value="<?php echo e(old('lokasi_kota',$event->lokasi_kota??'')); ?>"
                 placeholder="Surabaya" class="form-input px-4 py-3 text-sm" required>
        </div>
        <div>
          <label class="form-label">Venue / Tempat</label>
          <input type="text" name="venue" value="<?php echo e(old('venue',$event->venue??'')); ?>"
                 placeholder="Nama gedung / lokasi" class="form-input px-4 py-3 text-sm" required>
        </div>
      </div>

      <div>
        <label class="form-label">Deskripsi Event</label>
        <textarea name="deskripsi" rows="5" placeholder="Ceritakan tentang event kamu..."
                  class="form-input px-4 py-3 text-sm resize-none"><?php echo e(old('deskripsi',$event->deskripsi??'')); ?></textarea>
      </div>
      ...
      <div>
        ...
        <div id="cover-dropzone"
          ...
          <div id="cover-preview-wrap" class="<?php echo e(($event && $event->gambar_cover) ? '' : 'hidden'); ?> absolute inset-0">
            <img id="cover-preview-img" src="<?php echo e($event && $event->gambar_cover ? $event->cover_url : ''); ?>" class="w-full h-full object-cover">
            <div class="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
              <button type="button" onclick="event.stopPropagation();document.getElementById('cover-file-input').click()"
                      class="bg-white text-navy-mid font-semibold text-xs px-4 py-2 rounded-lg hover:bg-gray-100 transition">Ganti Foto</button>
              <button type="button" onclick="event.stopPropagation();openCropper()"
                      class="bg-gold text-navy-deep font-semibold text-xs px-4 py-2 rounded-lg hover:bg-gold-light transition">️ Crop</button>
            </div>
          </div>

          <div id="cover-empty-state" class="<?php echo e(($event && $event->gambar_cover) ? 'hidden' : ''); ?> flex flex-col items-center justify-center h-full py-12 pointer-events-none">
        ...
        <div id="cover-actions" class="<?php echo e(($event && $event->gambar_cover) ? '' : 'hidden'); ?> flex items-center gap-2 mt-2">
          <button type="button" onclick="openCropper()" class="flex items-center gap-1.5 text-xs font-semibold text-navy-mid bg-navy-mid/10 hover:bg-navy-mid/20 px-3 py-1.5 rounded-lg transition">️ Crop & Resize</button>
          <button type="button" onclick="removeCover()" class="flex items-center gap-1.5 text-xs font-semibold text-red-500 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg transition"> Hapus</button>
      ...
      <div id="cropper-modal" class="fixed inset-0 z-[70] hidden bg-black/80 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl">
          ...
          <div class="flex gap-3 px-5 py-4 border-t border-gray-100">
            <button type="button" onclick="closeCropper()" class="flex-1 border border-gray-200 text-gray-600 font-semibold text-sm py-2.5 rounded-xl hover:bg-gray-50 transition">Batal</button>
            <button type="button" onclick="applyCrop()" class="flex-1 bg-navy-mid text-white font-bold text-sm py-2.5 rounded-xl hover:bg-navy-deep transition"> Terapkan Crop</button>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div id="gallery-grid" class="grid grid-cols-3 sm:grid-cols-4 gap-3 mb-3">
          <?php if($event && $event->galleries->isNotEmpty()): ?>
            <?php $__currentLoopData = $event->galleries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $gal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="gallery-item relative aspect-square rounded-xl overflow-hidden bg-gray-100 group" data-gallery-id="<?php echo e($gal->id); ?>">
              <input type="hidden" name="gallery_keep_ids[]" value="<?php echo e($gal->id); ?>">
              <img src="<?php echo e($gal->url); ?>" class="w-full h-full object-cover">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      <div class="flex items-center justify-between mb-5">
        <h2 class="font-bold text-navy-deep text-lg"> Kategori Tiket</h2>
        <button type="button" class="add-btn" onclick="tambahTicket()">+ Tambah Tiket</button>
      </div>
      <div id="ticketContainer" class="space-y-3">
        <?php if($event && $event->ticketCategories->isNotEmpty()): ?>
          <?php $__currentLoopData = $event->ticketCategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <div class="item-row flex gap-3 items-center">
            <input type="text" name="nama_kategori[]" value="<?php echo e($cat->nama_kategori); ?>" placeholder="Nama kategori" class="form-input flex-1 px-3 py-2.5 text-sm">
            <input type="number" name="harga[]" value="<?php echo e($cat->harga); ?>" placeholder="Harga (Rp)" class="form-input w-36 px-3 py-2.5 text-sm">
            <input type="number" name="kuota[]" value="<?php echo e($cat->kuota); ?>" placeholder="Kuota" class="form-input w-28 px-3 py-2.5 text-sm">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      ...
      <div id="lineupContainer" class="space-y-3">
        <?php if($event && $event->lineups->isNotEmpty()): ?>
          <?php $__currentLoopData = $event->lineups; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $lu): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <div class="item-row flex gap-3 items-center">
            <input type="text" name="lineup_nama[]" value="<?php echo e($lu->nama); ?>" placeholder="Nama artis / pembicara" class="form-input flex-1 px-3 py-2.5 text-sm">
            <label class="flex items-center gap-2 cursor-pointer flex-shrink-0">
              <input type="checkbox" name="lineup_headliner[<?php echo e($i); ?>]" value="1" <?php echo e($lu->is_headliner?'checked':''); ?> class="accent-yellow-400">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      ...
      <div id="faqContainer" class="space-y-3">
        <?php if($event && $event->faqs->isNotEmpty()): ?>
          ...
          <div class="item-row">
            <div class="flex gap-3 items-start">
              <div class="flex-1 space-y-2">
                <input type="text" name="faq_pertanyaan[]" value="<?php echo e($faq->pertanyaan); ?>" placeholder="Pertanyaan..." class="form-input px-3 py-2.5 text-sm">
                <textarea name="faq_jawaban[]" rows="2" placeholder="Jawaban..." class="form-input px-3 py-2.5 text-sm resize-none"><?php echo e($faq->jawaban); ?></textarea>
              </div>
    ...
    <div class="flex gap-3">
      <button type="submit" class="flex-1 bg-gold text-navy-deep font-bold py-4 rounded-xl hover:bg-gold-light transition-all text-sm shadow-sm">
        <?php echo e($event ? ' Simpan Perubahan' : ' Buat Event'); ?>

      </button>
      <a href="<?php echo e(route('pengelola.dashboard')); ?>" class="bg-gray-100 text-gray-700 font-semibold px-6 py-4 rounded-xl hover:bg-gray-200 transition-all text-sm">Batal</a>
    </div>
  </form>
</div>
...
<script>
let lineupIdx = <?php echo e($event ? $event->lineups->count() : 0); ?>;
...
function tambahLineup() {
  const ph = document.getElementById('lineupPlaceholder');
  if (ph) ph.remove();
  const i = lineupIdx++;
...
function tambahFaq() {
  const ph = document.getElementById('faqPlaceholder');
  if (ph) ph.remove();
...
function onCoverFileSelected(input) {
  ...
  loadImageFromFile(input.files[0], img => {
    cropSourceImage = img; cropOffsetX = 0; cropOffsetY = 0;
...
function drawCropPreview() {
  ...
  const sw = cropSourceImage.width * scale, sh = cropSourceImage.height * scale;
...
function applyCrop() {
  ...
  out.width = COVER_W; out.height = COVER_H;
  const ctx = out.getContext('2d');
  const dispW = document.getElementById('crop-canvas').width;
  const dispH = document.getElementById('crop-canvas').height;
  const scaleBase = Math.max(dispW / cropSourceImage.width, dispH / cropSourceImage.height);
  const scale = scaleBase * zoom;
  const srcX = -cropOffsetX / scale, srcY = -cropOffsetY / scale;
  const srcW = dispW / scale, srcH = dispH / scale;
...
function onGalleryFilesSelected(input) {
  ...
  Array.from(input.files).slice(0, canAdd).forEach(file => addGalleryFile(file));
  input.value = '';
}
  ...
  reader.onload = e => {
    ...
    div.innerHTML = `
      ...
      <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-1">
        <button type="button" onclick="openGalleryCropper(${idx})" class="bg-gold text-navy-deep text-xs font-bold px-2 py-1 rounded-lg">️</button>
        <button type="button" onclick="removeGalleryNew(this,${idx})" class="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-lg"></button>
      </div>`;
    grid.insertBefore(div, addBtn);
    if (document.querySelectorAll('.gallery-item').length >= MAX_GALLERY)
      document.getElementById('gallery-add-btn').classList.add('hidden');
  };
  reader.readAsDataURL(file);
}
...
function removeGalleryExisting(btn) {
  ...
  item.querySelector('input[name="gallery_keep_ids[]"]').remove();
  item.remove();
...
function openGalleryCropper(idx) {
  ...
  galCropIdx = idx;
  const img = new Image();
  img.onload = () => { cropSourceImage = img; cropOffsetX = 0; cropOffsetY = 0; document.getElementById('crop-zoom').value = 1; openCropper(); };
  img.src = src;
}
...
window.applyCrop = function() {
  if (galCropIdx >= 0) {
    ...
    const dispW = document.getElementById('crop-canvas').width;
    const dispH = document.getElementById('crop-canvas').height;
    const scale = Math.max(dispW/cropSourceImage.width, dispH/cropSourceImage.height) * zoom;
    ctx.drawImage(cropSourceImage, -cropOffsetX/scale, -cropOffsetY/scale, dispW/scale, dispH/scale, 0, 0, 1200, 900);
    ...
    galCropIdx = -1; cropSourceImage = null;
    closeCropper();
  } else {
    _applyCrop();
  }
...
<?php echo $__env->make('layouts.pengelola', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ticketin-laravel\resources\views/pengelola/form-event.blade.php ENDPATH**/ ?>