...
<div class="max-w-3xl mx-auto px-6 py-10">
  ...
  <form method="POST" action="<?php echo e($event ? route('pengelola.event.update',$event) : route('pengelola.event.store')); ?>"
    ...
    <?php echo csrf_field(); ?> <?php if($event): ?> <?php echo method_field('PUT'); ?> <?php endif; ?>

    <!-- Info Dasar -->
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
      <h2 class="font-bold text-navy-deep text-lg">📋 Informasi Event</h2>
      <div><label class="form-label">Judul Event</label><input type="text" name="judul" value="<?php echo e(old('judul',$event->judul??'')); ?>" placeholder="Nama event kamu" class="form-input px-4 py-3 text-sm" required></div>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div><label class="form-label">Status</label>
          <select name="status" class="form-input px-4 py-3 text-sm">
            <?php $__currentLoopData = ['draft'=>'📝 Draft','published'=>'🟢 Published','cancelled'=>'❌ Cancelled']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v=>$l): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <option value="<?php echo e($v); ?>" <?php echo e(old('status',$event->status??'draft')===$v?'selected':''); ?>><?php echo e($l); ?></option>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
          </select>
        </div>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div><label class="form-label">Tanggal</label><input type="date" name="tanggal" value="<?php echo e(old('tanggal',$event?$event->tanggal_waktu->format('Y-m-d'):'')); ?>" class="form-input px-4 py-3 text-sm" required></div>
        <div><label class="form-label">Jam Mulai</label><input type="time" name="waktu" value="<?php echo e(old('waktu',$event?$event->tanggal_waktu->format('H:i'):'')); ?>" class="form-input px-4 py-3 text-sm" required></div>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div><label class="form-label">Kota</label><input type="text" name="lokasi_kota" value="<?php echo e(old('lokasi_kota',$event->lokasi_kota??'')); ?>" placeholder="Surabaya" class="form-input px-4 py-3 text-sm" required></div>
        <div><label class="form-label">Venue / Tempat</label><input type="text" name="venue" value="<?php echo e(old('venue',$event->venue??'')); ?>" placeholder="Nama gedung / lokasi" class="form-input px-4 py-3 text-sm" required></div>
      </div>
      <div><label class="form-label">Deskripsi Event</label><textarea name="deskripsi" rows="5" placeholder="Ceritakan tentang event kamu..." class="form-input px-4 py-3 text-sm resize-none"><?php echo e(old('deskripsi',$event->deskripsi??'')); ?></textarea></div>
        ...
        <div id="cover-dropzone"
          ...
          <div id="cover-preview-wrap" class="<?php echo e(($event && $event->gambar_cover) ? '' : 'hidden'); ?> absolute inset-0">
            <img id="cover-preview-img"
                 src="<?php echo e($event && $event->gambar_cover ? $event->cover_url : ''); ?>"
...
<?php echo $__env->make('layouts.pengelola', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Application\Software\XAMPP\htdocs\ticketin-laravel\resources\views/pengelola/form-event.blade.php ENDPATH**/ ?>