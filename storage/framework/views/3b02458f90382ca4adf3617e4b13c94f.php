...
<head>
  ...
  <title><?php echo $__env->yieldContent('title', 'TicketIn — Temukan Event Terbaik'); ?></title>
  ...
  <header class="fixed top-0 left-0 w-full bg-navy-mid text-white shadow-lg z-50">
    <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
      ...
      <nav class="hidden md:flex gap-8">
        <a href="<?php echo e(route('dashboard')); ?>" class="nav-link hover:text-gold transition-colors duration-200 font-medium">Beranda</a>
        <a href="<?php echo e(route('events.index')); ?>" class="nav-link hover:text-gold transition-colors duration-200 font-medium">Event</a>
        <a href="<?php echo e(route('tentang')); ?>" class="nav-link hover:text-gold transition-colors duration-200 font-medium">Tentang</a>
        <a href="<?php echo e(route('hubungi')); ?>" class="nav-link hover:text-gold transition-colors duration-200 font-medium">Hubungi Kami</a>
      </nav>
...
<?php if(auth()->guard()->check()): ?>
          <div class="hidden sm:block relative" id="user-dropdown-wrap">
            <button onclick="toggleDropdown()" class="flex items-center gap-2.5 bg-white/10 hover:bg-white/20 border border-white/10 px-3 py-1.5 rounded-full transition-colors duration-300 shadow-sm">
              <img src="<?php echo e(auth()->user()->avatar_url); ?>" class="w-7 h-7 rounded-full object-cover shadow-sm" alt="Avatar">
              <span class="text-sm font-semibold tracking-wide"><?php echo e(auth()->user()->nama_panggilan); ?></span>
            ...
            <div id="user-dropdown" class="hidden absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden">
              <a href="<?php echo e(route('profile.index')); ?>" class="flex items-center gap-2 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-navy-mid transition-colors">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                Profil Saya
              </a>
              <?php if(auth()->user()->isAdmin()): ?>
              <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center gap-2 px-4 py-3 text-sm text-red-600 hover:bg-red-50 transition-colors">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                👑 Admin Panel
              </a>
              <?php elseif(auth()->user()->isPengelola()): ?>
              <a href="<?php echo e(route('pengelola.dashboard')); ?>" class="flex items-center gap-2 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-navy-mid transition-colors">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                Dashboard EO
              </a>
              <?php elseif(!auth()->user()->eoApplication): ?>
              <a href="<?php echo e(route('eo.daftar')); ?>" class="flex items-center gap-2 px-4 py-3 text-sm text-gold hover:bg-yellow-50 transition-colors font-semibold">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                🎭 Daftar Jadi EO
              </a>
              <?php else: ?>
              <a href="<?php echo e(route('eo.status')); ?>" class="flex items-center gap-2 px-4 py-3 text-sm text-yellow-600 hover:bg-yellow-50 transition-colors">
        ...
        <?php else: ?>
          <a href="<?php echo e(route('login')); ?>" class="hidden sm:block">
    ...
    <div id="mobile-menu" class="bg-navy-mid border-t border-white/10">
      <nav class="flex flex-col px-6 py-4 gap-3">
        <a href="<?php echo e(route('dashboard')); ?>" class="text-gold font-medium">Beranda</a>
        <a href="<?php echo e(route('events.index')); ?>" class="hover:text-gold transition font-medium">Event</a>
        <a href="<?php echo e(route('tentang')); ?>" class="hover:text-gold transition font-medium">Tentang</a>
        <a href="<?php echo e(route('hubungi')); ?>" class="hover:text-gold transition font-medium">Hubungi Kami</a>
        <?php if(auth()->guard()->check()): ?>
          <hr class="border-white/10 my-2">
          <a href="<?php echo e(route('profile.index')); ?>" class="hover:text-gold transition font-medium">Profil Saya (<?php echo e(auth()->user()->nama_panggilan); ?>)</a>
          <?php if(auth()->user()->isPengelola()): ?>
            <a href="<?php echo e(route('pengelola.dashboard')); ?>" class="hover:text-gold transition font-medium">Dashboard Pengelola</a>
          <?php endif; ?>
          <form method="POST" action="<?php echo e(route('logout')); ?>">
            <?php echo csrf_field(); ?>
        ...
        <?php else: ?>
          <hr class="border-white/10 my-2">
          <a href="<?php echo e(route('login')); ?>" class="text-gold hover:text-gold-light transition font-bold">Masuk / Daftar</a>
        <?php endif; ?>
      </nav>
    </div>
  </header>
  ...
  <div id="toast-notif"
       ...
       style="min-width:280px; max-width:400px;
              background: <?php echo e(session('success') ? '#f0fdf4' : '#fef2f2'); ?>;
              border: 1.5px solid <?php echo e(session('success') ? '#86efac' : '#fca5a5'); ?>;
              color: <?php echo e(session('success') ? '#15803d' : '#dc2626'); ?>;">
    <span class="text-xl flex-shrink-0"><?php echo e(session('success') ? '✅' : '❌'); ?></span>
    <span class="flex-1"><?php echo e(session('success') ?? session('error')); ?></span>
...
<?php /**PATH D:\Application\Software\XAMPP\htdocs\ticketin-laravel\resources\views/layouts/app.blade.php ENDPATH**/ ?>