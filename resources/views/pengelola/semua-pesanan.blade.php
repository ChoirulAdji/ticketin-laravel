...
<div class="p-6 max-w-7xl mx-auto">
  ...
  <div class="flex items-center justify-between mb-6">
    <div>
      ...
      <p class="text-gray-400 text-sm mt-0.5">Total {{ $orders->total() }} pesanan ditemukan</p>
    </div>
    ...
    <form method="GET" class="flex gap-2">
      ...
      <select name="event_id" onchange="this.form.submit()" class="text-sm border border-gray-200 rounded-xl px-3 py-2 text-gray-600 outline-none focus:border-navy-mid bg-white max-w-[200px]">
        ...
        @foreach($events as $ev)
          <option value="{{ $ev->id }}" {{ request('event_id')==$ev->id?'selected':'' }}>{{ \Illuminate\Support\Str::limit($ev->judul,25) }}</option>
        @endforeach
      </select>
    </form>
  </div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-50">
            ...
            <tr class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-4 py-4">
                <p class="text-navy-deep text-xs font-medium max-w-[160px] truncate">{{ $order->event->judul }}</p>
              </td>
              <td class="px-4 py-4">
                <span class="font-mono text-xs text-navy-mid bg-navy-mid/10 px-2 py-1 rounded-lg">{{ $order->order_code }}</span>
              </td>
              <td class="px-4 py-4">
                <p class="text-xs text-navy-deep">{{ $order->total_qty }} tiket</p>
                <p class="text-xs text-gray-400">{{ \Illuminate\Support\Str::limit($order->ticket_summary,22) }}</p>
              </td>
              <td class="px-4 py-4">
