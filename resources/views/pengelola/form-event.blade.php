    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="date" name="tanggal"
                 value="{{ old('tanggal',$event?$event->tanggal_waktu->format('Y-m-d'):'') }}"
                 class="form-input px-4 py-3 text-sm" required>
        </div>
        <div>
          <label class="form-label">Jam Mulai</label>
          <input type="time" name="waktu"
                 value="{{ old('waktu',$event?$event->tanggal_waktu->format('H:i'):'') }}"
                 class="form-input px-4 py-3 text-sm" required>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="lokasi_kota" value="{{ old('lokasi_kota',$event->lokasi_kota??'') }}"
      ...
      <div>
        ...
        <textarea name="deskripsi" rows="5" placeholder="Ceritakan tentang event kamu..."
                  class="form-input px-4 py-3 text-sm resize-none">{{ old('deskripsi',$event->deskripsi??'') }}</textarea>
      </div>
      ...
      <div>
        ...
        <div id="cover-dropzone"
          ...
          <div id="cover-preview-wrap" class="{{ ($event && $event->gambar_cover) ? '' : 'hidden' }} absolute inset-0">
            <img id="cover-preview-img" src="{{ $event && $event->gambar_cover ? $event->cover_url : '' }}" class="w-full h-full object-cover">
