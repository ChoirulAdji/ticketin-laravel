@extends('layouts.app')
@section('title', 'TicketIn — ' . $event->judul)
...
<div class="pt-24 max-w-7xl mx-auto px-6 py-10 pb-28 lg:pb-10">
  <div class="flex flex-col lg:flex-row gap-10 items-start">
    ...
    <div class="flex-1 w-full min-w-0">
      ...
      <!-- Category + Title -->
      <div class="mb-6">
        <span class="inline-block bg-navy-mid/10 text-navy-mid text-xs font-bold px-3 py-1.5 rounded-md mb-3 uppercase tracking-wider">{{ $event->kategori }}</span>
        <h1 class="text-3xl md:text-4xl font-extrabold text-navy-deep leading-tight mb-2">{{ $event->judul }}</h1>
      ...
      <div class="mt-3 mb-4 px-4 md:px-0">
        <div class="flex gap-2 overflow-x-auto pb-2 hide-scrollbar">
          ...
          @foreach($event->galleries as $i => $gal)
          <button type="button" onclick="openLightbox({{ $i + 1 }})"
            class="flex-shrink-0 w-24 h-16 rounded-xl overflow-hidden ring-1 ring-gray-200 hover:ring-gold transition shadow">
            <img src="{{ $gal->url }}" class="w-full h-full object-cover hover:scale-105 transition-transform duration-300">
          </button>
          @endforeach
        </div>
      </div>
      ...
      <div class="tab-panel active" id="tab-info">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
          <div class="bg-white border border-gray-100 rounded-xl p-4 text-center shadow-sm">
            <p class="text-2xl font-extrabold text-navy-mid">{{ $event->lineups->count() ?: '–' }}</p>
            <p class="text-xs text-gray-500 mt-1">Artis / Pembicara</p>
          </div>
          <div class="bg-white border border-gray-100 rounded-xl p-4 text-center shadow-sm">
            <p class="text-2xl font-extrabold text-navy-mid">{{ $event->ticketCategories->count() }}</p>
          ...
          <div class="bg-white border border-gray-100 rounded-xl p-4 text-center shadow-sm col-span-2 sm:col-span-1">
            <p class="text-2xl font-extrabold text-navy-mid">{{ $event->tanggal_waktu->format('H:i') }}</p>
      ...
      <div class="tab-panel" id="tab-lineup">
        <div class="space-y-4">
          @if($event->lineups->isNotEmpty())
            @foreach($event->lineups as $artis)
            <div class="bg-white rounded-xl p-4 border border-gray-100 shadow-sm flex items-center gap-4 hover:border-navy-mid/30 transition-colors">
              <div class="w-16 h-16 rounded-full bg-navy-mid/10 flex items-center justify-center flex-shrink-0 text-navy-mid font-bold text-xl">
                {{ strtoupper(substr($artis->nama, 0, 1)) }}
              </div>
              <div class="flex-1">
                <p class="font-bold text-navy-deep text-base">{{ $artis->nama }}</p>
                <p class="text-sm text-gray-500">Penampilan Spesial</p>
              </div>
              @if($artis->is_headliner)
        ...
        <div class="space-y-3">
          @if($event->faqs->isNotEmpty())
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div class="faq-ans px-5 pb-4 text-sm text-gray-500 leading-relaxed">{{ $faq->jawaban }}</div>
            </div>
            @endforeach
          @else
      ...
      <div class="tab-panel" id="tab-ulasan">
        ...
        {{-- Form review (hanya jika bisa review) --}}
        @auth
          @if($userReview)
            <div class="bg-green-50 border border-green-200 rounded-xl px-5 py-4 mb-6 flex items-start justify-between gap-4">
              <div>
                ...
                @if($userReview->ulasan)
                  <p class="text-sm text-gray-600">{{ $userReview->ulasan }}</p>
                @endif
              </div>
              <form method="POST" action="{{ route('events.review.destroy', $event) }}" onsubmit="return confirm('Hapus ulasan ini?')">
                @csrf @method('DELETE')
          ...
          @elseif($userCanReview)
            <div class="bg-white border border-gray-100 rounded-2xl p-6 mb-6 shadow-sm" id="review-form-wrap">
              ...
              <form method="POST" action="{{ route('events.review.store', $event) }}" id="review-form">
                @csrf
                {{-- Star Rating --}}
          ...
          @else
            <div class="bg-gray-50 border border-dashed border-gray-200 rounded-xl px-5 py-4 mb-6 text-sm text-gray-500 text-center">
              ...
              @else
                <a href="{{ route('login') }}" class="text-navy-mid font-semibold hover:underline">Masuk</a> untuk menulis ulasan.
              @endauth
            </div>
          @endif
        @else
          ...
          <div class="space-y-4">
            ...
            <div class="review-card bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
              <div class="flex items-start gap-3">
                <img src="{{ $review->user->avatar_url }}" alt="{{ $review->user->nama_panggilan }}"
                     class="w-9 h-9 rounded-full object-cover flex-shrink-0 ring-2 ring-gray-100">
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between gap-2 flex-wrap">
                    <p class="text-sm font-semibold text-navy-deep">{{ $review->user->nama_panggilan }}</p>
                  ...
                  @if($review->ulasan)
                    <p class="text-sm text-gray-600 leading-relaxed mt-1">{{ $review->ulasan }}</p>
                  @endif
                </div>
              </div>
            </div>
      ...
      <div class="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100 p-6 lg:sticky lg:top-28">
        <div class="space-y-5">
          <div class="flex items-start gap-4">
            ...
            <div>
              <p class="font-bold text-navy-deep">{{ $event->tanggal_waktu->translatedFormat('l, d F Y') }}</p>
              <p class="text-sm text-gray-500 mt-0.5">{{ $event->tanggal_waktu->format('H:i') }} WIB</p>
            </div>
          </div>
          <div class="flex items-start gap-4">
            <div class="w-12 h-12 rounded-xl bg-navy-mid/5 flex items-center justify-center flex-shrink-0 text-navy-mid">
            ...
            <div>
              <p class="font-bold text-navy-deep">{{ $event->lokasi_kota }}</p>
              <p class="text-sm text-gray-500 mt-0.5">{{ $event->venue }}</p>
...
<div class="buy-bar lg:hidden">
  <div class="flex items-center justify-between gap-4">
    ...
    @else
      <a href="{{ route('login') }}" class="flex-1 bg-gold text-navy-deep text-center font-bold py-3 rounded-xl text-sm shadow-md">
        Masuk untuk Beli
      </a>
    @endauth
  </div>
...
<script>
  ...
  const lightboxImages = [
    '{{ $event->cover_url }}',
    @foreach($event->galleries as $gal)'{{ $gal->url }}',@endforeach
  ];
  let lbIndex = 0;
