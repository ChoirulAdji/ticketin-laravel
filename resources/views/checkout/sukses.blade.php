  ...
  <div class="step active" id="step-instruksi">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-5">
      ...
      <div class="flex items-center justify-between mb-5">
        <div>
          ...
          <p class="text-gray-500 text-sm mt-0.5">Order: <span class="font-mono font-bold text-navy-mid">{{ $order->order_code }}</span></p>
        </div>
        {{-- Countdown --}}
      ...
      <div class="bg-navy-mid/5 border border-navy-mid/10 rounded-xl p-4 mb-5">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-extrabold text-navy-deep mt-0.5">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</p>
          </div>
          <div class="text-right">
            <p class="text-xs text-gray-500">Metode</p>
            <p class="font-bold text-navy-mid text-sm mt-0.5">{{ $order->metode_bayar }}</p>
          </div>
        </div>
      </div>
      ...
      @if($isTransfer)
        ...
        <div class="mb-5">
          ...
          <div class="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5">
            <div>
              <p class="text-xs text-gray-400 mb-1">{{ strtoupper($order->metode_bayar) }} Virtual Account</p>
              <p class="text-xl font-extrabold text-navy-deep tracking-wider" id="va-number">{{ $vaNumber }}</p>
            </div>
        ...
        <div class="space-y-2 mb-5">
          @if($metode === 'bca')
            @foreach(['Buka aplikasi myBCA atau KlikBCA Internet Banking','Pilih menu Pembayaran → Virtual Account','Masukkan Nomor VA di atas: '.$vaNumber,'Periksa detail pembayaran, pastikan nama dan nominal sesuai','Masukkan PIN/OTP dan konfirmasi pembayaran','Simpan bukti bayar — tiket akan dikirim ke email kamu'] as $i => $step)
            <div class="flex items-start gap-3 text-sm text-gray-600">
              <span class="w-6 h-6 rounded-full bg-navy-mid text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{{ $i+1 }}</span>
              {{ $step }}
            </div>
            @endforeach
          @elseif($metode === 'mandiri')
            @foreach(['Buka aplikasi Livin by Mandiri','Pilih Pembayaran → Multipayment','Masukkan kode perusahaan: 88277 dan Nomor VA: '.$vaNumber,'Ikuti instruksi hingga pembayaran selesai','Simpan bukti bayar sebagai referensi'] as $i => $step)
            <div class="flex items-start gap-3 text-sm text-gray-600">
              <span class="w-6 h-6 rounded-full bg-navy-mid text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{{ $i+1 }}</span>
              {{ $step }}
            </div>
            @endforeach
          @else
            @foreach(['Buka aplikasi mobile banking BNI','Pilih Transfer → Virtual Account','Masukkan Nomor VA: '.$vaNumber,'Konfirmasi pembayaran dan simpan bukti'] as $i => $step)
            <div class="flex items-start gap-3 text-sm text-gray-600">
              <span class="w-6 h-6 rounded-full bg-navy-mid text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{{ $i+1 }}</span>
              {{ $step }}
            </div>
      ...
      @elseif($isEwallet)
        ...
        <div class="space-y-2 mb-5">
          @if($metode === 'gopay')
            @foreach(['Buka aplikasi Gojek di smartphone kamu','Tap ikon GoPay → Scan QR','Arahkan kamera ke QR Code di atas','Periksa nominal Rp '.number_format($order->total_harga,0,',','.').' dan konfirmasi','Pembayaran selesai!'] as $i => $step)
            <div class="flex items-start gap-3 text-sm text-gray-600">
              <span class="w-6 h-6 rounded-full bg-navy-mid text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{{ $i+1 }}</span>{{ $step }}
            </div>
            @endforeach
          @elseif($metode === 'ovo')
            @foreach(['Buka aplikasi OVO di smartphone kamu','Tap icon Scan di halaman utama','Scan QR Code di atas','Masukkan PIN OVO untuk konfirmasi','Pembayaran berhasil!'] as $i => $step)
            <div class="flex items-start gap-3 text-sm text-gray-600">
              <span class="w-6 h-6 rounded-full bg-navy-mid text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{{ $i+1 }}</span>{{ $step }}
            </div>
            @endforeach
          @else
            @foreach(['Buka aplikasi m-banking atau dompet digital apapun','Pilih menu Scan QR / QRIS','Scan QR Code di atas','Konfirmasi nominal dan selesaikan pembayaran'] as $i => $step)
            <div class="flex items-start gap-3 text-sm text-gray-600">
              <span class="w-6 h-6 rounded-full bg-navy-mid text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{{ $i+1 }}</span>{{ $step }}
            </div>
            @endforeach
      ...
      @else
        {{-- Default / Transfer Bank biasa --}}
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-5 text-sm text-gray-600">
          Silakan lakukan pembayaran sebesar <strong class="text-navy-deep">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</strong> sesuai metode yang dipilih. Tim kami akan memverifikasi pembayaran dalam 1x24 jam.
        </div>
      @endif
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-5">
      ...
      <div class="flex gap-4 mb-4">
        <img src="{{ $order->event->cover_url }}" class="w-14 h-14 rounded-xl object-cover flex-shrink-0">
        <div>
          <p class="font-bold text-navy-deep text-sm">{{ $order->event->judul }}</p>
      ...
      <div class="border-t border-gray-100 pt-3 space-y-1.5 text-sm">
        ...
        <div class="flex justify-between text-gray-600">
          <span>{{ $item->qty }}× {{ $item->ticketCategory->nama_kategori }}</span>
          <span>Rp {{ number_format($item->subtotal, 0, ',', '.') }}</span>
        </div>
        @endforeach
        <div class="flex justify-between font-bold text-navy-deep border-t border-gray-100 pt-2 mt-2">
          <span>Total</span>
          <span class="text-gold">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</span>
        </div>
      </div>
    </div>
  ...
  <div class="step" id="step-sukses">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center mb-5">
      ...
      <div class="flex gap-3 justify-center">
        <a href="{{ route('profile.index') }}" class="bg-navy-mid text-white font-bold px-5 py-2.5 rounded-xl hover:bg-navy-deep transition-all text-sm">Lihat Semua Tiket</a>
        <a href="{{ route('dashboard') }}" class="bg-gray-100 text-gray-700 font-bold px-5 py-2.5 rounded-xl hover:bg-gray-200 transition-all text-sm">Ke Beranda</a>
      </div>
    </div>
    ...
    <div class="e-ticket p-6 shadow-xl">
      <div class="flex justify-between items-start mb-4">
        <div>
          ...
          <h3 class="text-lg font-extrabold text-gold leading-tight">{{ $order->event->judul }}</h3>
        </div>
      ...
      <div class="grid grid-cols-2 gap-3 text-xs mb-4">
        <div><p class="text-white/50 mb-1">Tanggal</p><p class="font-bold text-white">{{ $order->event->tanggal_waktu->format('d M Y') }}</p></div>
        <div><p class="text-white/50 mb-1">Waktu</p><p class="font-bold text-white">{{ $order->event->tanggal_waktu->format('H:i') }} WIB</p></div>
        <div><p class="text-white/50 mb-1">Lokasi</p><p class="font-bold text-white">{{ $order->event->lokasi_kota }}</p></div>
        <div><p class="text-white/50 mb-1">Tiket</p><p class="font-bold text-white">{{ $order->total_qty }} Tiket</p></div>
        <div><p class="text-white/50 mb-1">Tipe</p><p class="font-bold text-white text-xs">{{ $order->ticket_summary }}</p></div>
        <div><p class="text-white/50 mb-1">Pembayaran</p><p class="font-bold text-gold">Rp {{ number_format($order->total_harga,0,',','.') }}</p></div>
      </div>
...
<script>
  ...
  function konfirmasiPembayaran() {
    ...
    fetch('/checkout/konfirmasi/{{ $order->order_code }}', {
      method: 'POST',
      headers: {
  ...
  function shareTicket() {
    if (navigator.share) {
      navigator.share({ title: 'Tiket TicketIn - {{ $order->event->judul }}', url: location.href });
    } else {
