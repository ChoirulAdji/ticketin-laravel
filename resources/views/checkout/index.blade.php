@extends('layouts.app')
@section('title', 'Checkout — ' . $event->judul)
...
<div class="pt-24 max-w-7xl mx-auto px-6 py-10 pb-28 lg:pb-10">
  ...
  <form id="checkoutForm" method="POST" action="{{ route('checkout.proses', $event) }}">
    @csrf
    <div class="flex flex-col lg:flex-row gap-8">
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" name="nama" value="{{ auth()->user()->nama_lengkap }}" placeholder="Nama sesuai KTP" class="form-input px-4 py-2.5 text-sm"/>
              </div>
              <div>
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Email *</label>
                <input type="email" value="{{ auth()->user()->email }}" readonly class="form-input px-4 py-2.5 text-sm"/>
              ...
              <div>
                ...
                <div class="flex">
                  ...
                  <input type="tel" name="no_hp" value="{{ ltrim(auth()->user()->no_hp ?? '', '0') }}" placeholder="812xxxxxxxx" class="form-input flex-1 px-4 py-2.5 text-sm rounded-l-none" style="border-radius:0 12px 12px 0"/>
                </div>
              </div>
              <div>
        ...
        <div class="step-panel" id="panel2">
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
              @foreach([['BCA','bg-blue-600','BCA'],['Mandiri','bg-yellow-500','M'],['BNI','bg-orange-600','BNI']] as [$nama,$bg,$label])
              <div class="pay-method {{ $loop->first ? 'selected':'' }} p-4 flex items-center gap-3" onclick="selectPay(this,'{{ $nama }}')">
                <div class="w-10 h-10 {{ $bg }} rounded-lg flex items-center justify-center text-white font-extrabold text-xs">{{ $label }}</div>
                <div><p class="font-bold text-navy-deep text-xs">{{ $nama }}</p><p class="text-[11px] text-gray-400 mt-0.5">Virtual Account</p></div>
              </div>
              @endforeach
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
              @foreach([['OVO','bg-purple-600','OVO','Potong Saldo'],['GoPay','bg-green-500','GP','Aplikasi Gojek'],['QRIS','bg-navy-mid','QR','Scan Semua Bank']] as [$nama,$bg,$label,$desc])
              <div class="pay-method p-4 flex items-center gap-3" onclick="selectPay(this,'{{ $nama }}')">
                <div class="w-10 h-10 {{ $bg }} rounded-lg flex items-center justify-center text-white font-bold text-xs">{{ $label }}</div>
                <div><p class="font-bold text-navy-deep text-xs">{{ $nama }}</p><p class="text-[11px] text-gray-400 mt-0.5">{{ $desc }}</p></div>
              </div>
              @endforeach
            </div>
      ...
      <aside class="w-full lg:w-[380px] flex-shrink-0">
        <div class="summary-card p-6 text-white lg:sticky lg:top-24 shadow-xl">
          ...
          <img src="{{ $event->cover_url }}" class="w-full h-32 object-cover rounded-xl mb-4 opacity-90"/>
          <h3 class="font-extrabold text-lg leading-tight mb-1 line-clamp-2">{{ $event->judul }}</h3>
          <p class="text-white/70 text-xs flex items-center gap-1.5 mb-1">
            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            {{ $event->tanggal_waktu->format('d M Y') }} · {{ $event->tanggal_waktu->format('H:i') }} WIB
          </p>
          ...
          <div class="space-y-3 text-sm">
            ...
            <div class="flex justify-between text-white/80">
              <span>{{ $item['qty'] }}× {{ $item['category']->nama_kategori }}</span>
            ...
            <div class="flex justify-between items-end font-extrabold border-t border-white/15 pt-4">
              ...
              <span class="text-gold text-xl">Rp {{ number_format($total,0,',','.') }}</span>
            </div>
          </div>
...
<div class="buy-bar lg:hidden">
  <div class="flex items-center justify-between gap-4">
    <div>
      ...
      <p class="text-lg font-extrabold text-navy-deep">Rp {{ number_format($total,0,',','.') }}</p>
    </div>
