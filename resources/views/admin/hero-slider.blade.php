...
<div class="max-w-5xl mx-auto px-4 py-8">
  ...
  <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
      <h3 class="font-bold text-navy-deep">Slide Aktif ({{ $sliders->count() }})</h3>
    ...
    @else
      <ul id="slider-list" class="divide-y divide-gray-50">
        ...
        <li class="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition" data-id="{{ $slider->id }}">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-blue-400 truncate">🔗 {{ $slider->url_tujuan }}</p>
            @endif
          </div>
          ...
          {{-- Toggle aktif --}}
          <form method="POST" action="{{ route('admin.hero-slider.toggle', $slider) }}">
            @csrf
            <button type="submit"
          ...
          {{-- Hapus --}}
          <form method="POST" action="{{ route('admin.hero-slider.destroy', $slider) }}"
