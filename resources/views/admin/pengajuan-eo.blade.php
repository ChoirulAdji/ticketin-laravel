...
@section('title','Verifikasi Pengajuan EO')
...
<div class="p-6 max-w-7xl mx-auto">
  ...
  <div class="flex items-center justify-between mb-6">
    <div>
      ...
      <p class="text-gray-400 text-sm mt-0.5">Total {{ $pengajuan->total() }} pengajuan</p>
    </div>
    <form method="GET">
  ...
  <div class="space-y-4">
    ...
    <div class="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:border-navy-mid/20 transition-colors">
      <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
        ...
        <div class="flex items-center gap-3 flex-1 min-w-0">
          <img src="{{ $app->user->avatar_url }}" class="w-12 h-12 rounded-full object-cover flex-shrink-0">
          <div class="min-w-0">
            <p class="font-bold text-navy-deep">{{ $app->user->nama_lengkap }}</p>
            <p class="text-gray-400 text-sm">{{ $app->user->email }}</p>
            <p class="text-gray-400 text-xs mt-0.5">Dikirim {{ $app->created_at->translatedFormat('d M Y, H:i') }}</p>
          </div>
        </div>

        {{-- Org Info --}}
        <div class="flex-1 min-w-0">
          <p class="font-bold text-navy-mid">{{ $app->nama_organisasi }}</p>
          <p class="text-gray-500 text-sm">{{ ucfirst($app->jenis_entitas) }} · {{ ucfirst($app->skala_event) }}</p>
          <p class="text-gray-400 text-xs truncate">{{ $app->alamat_organisasi }}</p>
        </div>
        ...
        <div class="flex gap-2 flex-shrink-0">
          <button onclick="lihatDetail({{ $app->id }})"
    ...
    {{-- Data tersembunyi --}}
    <div id="app-data-{{ $app->id }}" class="hidden"
         data-nama="{{ $app->user->nama_lengkap }}"
         data-email="{{ $app->user->email }}"
         data-org="{{ $app->nama_organisasi }}"
         data-jenis="{{ ucfirst($app->jenis_entitas) }}"
         data-skala="{{ ucfirst($app->skala_event) }}"
         data-alamat="{{ $app->alamat_organisasi }}"
         data-hp="{{ $app->no_hp_bisnis }}"
         data-website="{{ $app->website ?? '-' }}"
         data-npwp="{{ $app->npwp ?? '-' }}"
         data-bank="{{ strtoupper($app->bank) }}"
         data-rek="{{ $app->nomor_rekening }}"
         data-nama-rek="{{ $app->nama_rekening }}"
         data-dokumen="{{ $app->dokumen_url ?? '' }}"
         data-status="{{ $app->status }}"
         data-avatar="{{ $app->user->avatar_url }}">
    </div>
    @endforeach
  </div>
    ...
    <div class="p-5">
      ...
      <div class="space-y-3 text-sm">
        ...
        <div class="flex justify-between gap-4 py-2 border-b border-gray-50">
          ...
          <span id="{{ $id }}" class="font-semibold text-navy-deep text-right"></span>
        </div>
        @endforeach
